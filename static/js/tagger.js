...
(function (root, factory, undefined) {
  ...
  tagger.fn = tagger.prototype = {
    ...
    _add_events: function () {
      ...
      this._ul.addEventListener("click", function (event) {
        if (event.target.className.match(/close/)) {
          ...
          //Focus new input when clicking in the whitespace of the Tagger instance
      ...
      this._new_input_tag.addEventListener("keydown", function (event) {
        if (
          ...
          event.preventDefault();
        } else if (event.keyCode === 8 && !self._new_input_tag.value) {
          // backspace
          ...
          event.preventDefault();
        } else if (self._tag_limit() && event.keyCode !== 9) {
          // tab
          event.preventDefault();
        }
      });
      ...
      this._new_input_tag.addEventListener("input", function (event) {
        ...
        if (self._tag_selected(value)) {
          ...
          if (
            typeof self._settings.completion.list === "function" &&
            value.length >= min
          ) {
            self.complete(value);
          }
    ...
    _tag_selected: function (tag) {
      if (this._last_completion) {
        if (this._last_completion.includes(tag)) {
          ...
          return (
            this._last_completion.filter(function (test_tag) {
    ...
    _toggle_completion: function (toggle) {
      if (toggle) {
        this._new_input_tag.setAttribute(
          "list",
          "tagger-completion-" + this._id
        );
      } else {
        this._new_input_tag.setAttribute(
          "list",
          "tagger-completion-disabled-" + this._id
        );
      }
    },
    // --------------------------------------------------------------------------------------
    ...
    _tag_limit: function () {
      return (
        this._settings.tag_limit > 0 &&
        this._tags.length >= this._settings.tag_limit
      );
    },
    // --------------------------------------------------------------------------------------
